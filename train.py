...
def main(env_name, num_episodes, gamma, lam, kl_targ, batch_size, hid1_mult, init_logvar):
    ...
    '''
    yaml_val_model= val_func.to_yaml()# writing the yaml model to the yaml file
    with open('value_model.yaml', 'w') as yaml_val_file:
        yaml_val_file.write(yaml_val_model)
    yaml_policy_model= policy.to_yaml()# writing the yaml model to the yaml file
    with open('policy_model.yaml', 'w') as yaml_pol_file:
        yaml_pol_file.write(yaml_policy_model)
    '''
    ...
    if save_weights_flag == 1:
        policy_model.save('trpo.h5',save_formate='tf')
        policy_model.save_weights('myModeltrpo.h5')
        save_weights_flag = 0
    print('347')
    input('')
